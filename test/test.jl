...
# level_set_contour(grid,f,levels=levels, clabels=true)
...
cell_index, X, C = cal_coeff_2D(grid,ϕ,5)
...
clouds, Cell_num = sample_2D(cell_index, C, 5, grid)
...
ϕ_new = closest_point_2D(clouds, Cell_num, C, 5, ϕ, grid)
